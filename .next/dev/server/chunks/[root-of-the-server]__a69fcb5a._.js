...
const shouldSearchLawDb = (message)=>{
    ...
    // emojis-only or very short
    if (/^[:;\-\)\(\*\p{Emoji}]+$/u.test(trimmed)) return false;
    if (trimmed.length < 3) return false;
    // greetings / small talk
    if (/^(hi|hello|hey|bonjour|salut|salam|أهلا|مرحبا|good morning|good evening|thanks|thank you)\b/i.test(trimmed)) {
        // but if it's a question-like short sentence, allow search
        if (trimmed.endsWith("?") || trimmed.length > 20) return true;
        return false;
    }
    // legal keywords (multilingual)
    const legalKeywords = [
        "loi",
        "article",
        "code",
    ...
    // heuristics: question mark or long sentence
    if (trimmed.endsWith("?")) return true;
    if (trimmed.length > 20 && /\w+\s+\w+/.test(trimmed)) return true;
    return false;
};
const __TURBOPACK__default__export__ = shouldSearchLawDb;
}),
...
const POST = async (request)=>{
    ...
    const needsSearch = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$searchGate$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["shouldSearchLawDb"])(question);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a69fcb5a._.js.map